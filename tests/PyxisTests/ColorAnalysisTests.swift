...
final class ColorAnalysisTests: XCTestCase {
    func testMapsSolidColorsToClosetColors() throws {
        let cases: [(PyxisColor, ClosetColor)] = [
            (.black, .black),
            (.white, .white),
            (.gray, .gray),
            (.red, .red),
            ...
            (.yellow, .yellow),
            (.orange, .orange),
            (PyxisColor(red: 0.45, green: 0.25, blue: 0.12, alpha: 1), .brown),
            (PyxisColor(red: 0.02, green: 0.05, blue: 0.22, alpha: 1), .navy)
        ]
    ...
    func testPrefersSubstantialGarmentColorOverPlainPhotoBackground() throws {
        let url = try makeCenteredGarmentImage(
            background: .white,
            garment: PyxisColor(red: 0.02, green: 0.05, blue: 0.22, alpha: 1)
        )
        let result = try ColorAnalysisService().analyze(imageURL: url)
    ...
    private func makeImageFile(color: PyxisColor) throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("Pyxis-color-\(UUID().uuidString).png")
    ...
    private func makeHalfTransparentHalfBlueImage() throws -> URL {
        try makeCustomRGBAImage { x, _, width in
            x < width / 2
                ? PyxisColor(red: 1, green: 0, blue: 0, alpha: 0)
                : PyxisColor.blue
        }
    }
    ...
    private func makeFullyTransparentImage() throws -> URL {
        try makeCustomRGBAImage { _, _, _ in
            PyxisColor(red: 1, green: 1, blue: 1, alpha: 0)
        }
    }

    private func makeCenteredGarmentImage(background: PyxisColor, garment: PyxisColor) throws -> URL {
    ...
    private func makeCustomRGBAImage(
        ...
        colorAt: (Int, Int, Int) -> PyxisColor
    ) throws -> URL {
        #if canImport(UIKit)
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("Pyxis-custom-\(UUID().uuidString).png")
        try data.write(to: url)
        addTeardownBlock {
            try? FileManager.default.removeItem(at: url)
        }
